...
impl Data {
    ...
    {        
        ...
        let mut pixoo_cam = PixooCam::new();
        ...
        while !tachions.is_empty() 
        {
            let new_tachions: Vec<Vec2> = tachions.iter()
                                                  .flat_map(|t| 
                                                  {
                                                        ...
                                                        if point.y < self.dy 
                                                        {
                                                          ...
                                                          {
                                                              splits += 1;
                                                              self.hash.insert(point.l(), '|');
                                                              self.hash.insert(point.r(), '|');
                                                              vec![point.l(), point.r()]                                                              
                                                          } 
                                                            else 
                                                          {
            ...
            pixoo_cam.render_image(&self.hash,self.dy as usize,self.dy as usize);

            tachions = new_tachions;            
            tachions.dedup();            
        }

        pixoo_cam.save_all().await;
...
async fn part1(data:&[String])->usize
{    
    Data::new(data).count().await
}
...
#[allow(unused)]
pub async fn solve(data:&[String])
{    
    println!("Day7");
    println!("part1: {}",part1(data).await);
